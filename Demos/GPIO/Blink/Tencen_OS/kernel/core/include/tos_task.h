...
#define K_TASK_NAME_LEN_MAX             (20u)
...
__DEBUG__ __STATIC_INLINE__ void task_default_walker(k_task_t *task)
{
   char *state_str = "ABNORMAL";
	...
	tos_task_stack_draught_depth(task, &depth);
    ...
    tos_kprintf("%s    ", state_str);

    tos_kprintf("%04d    ", task->stk_size);
		
	tos_kprintf("%04d    ",depth);	
	tos_kprintf("%04d    ",task->stk_size - depth);		
	tos_kprintf("%02d%%  ",(depth*100)/task->stk_size);
	
    // tos_kprintln("stk base: 0x%p", task->stk_base);
    // tos_kprintln("stk top : 0x%p", task->stk_base + task->stk_size);

    tos_kprintf("%s", task->name);
    tos_kprintf("\n");
}
