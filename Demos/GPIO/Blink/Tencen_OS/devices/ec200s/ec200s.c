...
static int ec200s_reset(void)
{
#if ENABLE_EC200_HARDWARE_RESERT
    tos_sem_create_max(&module_ready_sem, 0, 1);

    HAL_GPIO_WritePin(EC200_RESET_GPIO_Port, EC20_RESET_Pin, GPIO_PIN_SET);
    osDelay(1000);
    HAL_GPIO_WritePin(EC200_RESET_GPIO_Port, EC20_RESET_Pin, GPIO_PIN_RESET);

    tos_sem_pend(&module_ready_sem, 30 * 1000);
    return 0;
#else
    at_echo_t echo;
    int try = 0;
    ...
    while (try++ < 10)
    {
        tos_at_cmd_exec(&echo, 1000, "AT+CFUN=1,1\r\n");
...
static int ec200s_check_ready(void)
{
    ...
    {
        tos_sem_pend(&module_ready_sem, 60 * 1000);
        return 0;
    }
}
...
static int ec200s_get_mac(char *mac_buff)
{
    at_echo_t echo;
    ...
    while (try++ < 10)
    {
        tos_at_cmd_exec(&echo, 1000, "AT+GSN\r\n");
        if (echo.status != AT_ECHO_STATUS_OK)
        {
            return -1;
        }
...
int ec200s_iccid_read(void)
{
    at_echo_t echo;
    char echo_buffer[32], *str;
    int try = 0;
    char iccid_buff[30] = {0};

    tos_at_echo_create(&echo, echo_buffer, sizeof(echo_buffer), NULL);
    while (try++ < 10)
    {

        tos_at_cmd_exec(&echo, 1000, "AT+QCCID\r\n");
        if (echo.status != AT_ECHO_STATUS_OK)
        {
            return -1;
        }

        str = strstr(echo.buffer, "+QCCID: ");
        if (!str)
        {
            return -1;
        }
        sscanf(str, "+QCCID: %s", iccid_buff);
        return 0;
    }
...
static int ec200s_get_net_time(uint32_t *time)
{
    ...
    while (try++ < 5)
    {
        ...
        tos_at_cmd_exec(&echo, 1000, "AT+CCLK?\r\n");
        if (echo.status != AT_ECHO_STATUS_OK)
        {
            return -1;
        }

        str = strstr(echo.buffer, "+CCLK:");
        if (!str)
        {
            return -1;
        }
        sscanf(str, "+CCLK: \"%d/%d/%d,%d:%d:%d\"", &year, &month, &date, &hours, &minutes, &seconds);
        if (year < 21)
        {
            return -1;
        }

        *time = mktime(year+2000, month, date, hours, minutes, seconds);
        return 0;
    }
...
static int ec200s_init(void)
{
    ...
    if (ec200s_iccid_read() != 0)
    {
        printf("read iccid fail\n");
        return -1;
    }
...
static int ec200s_connect(const char *ip, const char *port, sal_proto_t proto)
{
    ...
    if (id == -1)
    {
        tos_kprintln("channel alloc fail\n");
        return -1;
    }
...
sal_module_t sal_module_ec200s = {
    ...
    .get_net_time = ec200s_get_net_time,
};
